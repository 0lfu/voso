...
if (!empty($_GET["action"])) {
    if (!empty($_GET['u']) && !empty($_GET['reason']) && $_GET['action'] == "ban") {
        ...
        if ($result['role'] != "user") {
            header('Location: ../reports?e=cantbanadm');
            exit;
        }
        ...
        if ($con->query($ban_query) && $con->query($removeUser_query)) {
            header('Location: ../reports?s=banned');
            exit;
        }
    }
    if (!empty($_GET['id']) && $_GET['action'] == "delete-comment") {
        ...
        if ($con->query($deleteComment_query)) {
            header('Location: ../reports?s=delcom');
            exit;
        }
    }
    if (!empty($_GET['id']) && $_GET['action'] == "delete-report") {
        ...
        if ($con->query($deleteReport_query)) {
            header('Location: ../reports?s=delrep');
            exit;
        }
    }
    if (!empty($_GET['id']) && $_GET['action'] == "delete-player-report") {
        $deletePlayerReport_query = "DELETE FROM `player_reports` WHERE `video_id` = '$_GET[id]'";
        if ($con->query($deletePlayerReport_query)) {
            header('Location: ../reports?s=delrep');
            exit;
        }
    }
